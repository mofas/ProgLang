...
(define (envlookup env str)
  (cond [(null? env) 1(error "unbound variable during evaluation" str)]
  ...
  (cond [(var? e) 
        ...
        [(mlet? e)
         (let ([key (mlet-var e)]
               ...
               (eval-under-env (mlet-body e) (cons (cons key value) env)))]
        
        [(call? e)
         (let ([clo (eval-under-env (call-funexp e) env)]
               [arg (eval-under-env (call-actual e) env)])
           (if (closure? clo)
               (letrec ([func (closure-fun clo)]
                        [func_name (fun-nameopt func)]
                        [call_env (cons (cons (fun-formal func) arg) (closure-env clo))]) ;extend env with argument
                 (eval-under-env (fun-body func) 
                                 (if func_name                                     
                                     (cons (cons func_name clo) call_env) ;extend call-env with function name
                                     call_env)))                 
               (error "MUPL addition applied to non-number")))]
...
(define mupl-map
  (fun #f "map_func"
       (fun "iter" "list"
            (ifaunit (var "list") 
                     (aunit)
                     (apair (call (var "map_func") (fst (var "list")))
                            (call (var "iter") (snd (var "list"))))))))
                    
(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "i"
             (call (var "map") (fun #f "ele" (add (var "ele") (var "i")))))))
